...
from (
    select 
        ...
        --these are some extra snowflake commands for data auditing
        metadata$filename as _stg_file_name, --this is for filename
...
-- A surrogate key in a database is an artificially created unique identifier for each row in a table, typically generated by the system rather than derived from the actual business data. It acts as a substitute for a natural key.
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/location/delta-day02-2rows-add.csv t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
select * from stage_sch.location_stm;
